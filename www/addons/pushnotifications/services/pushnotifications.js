 ...
 */
.factory('$mmaPushNotifications', function($mmSite, $log, $mmText, $q, $cordovaDevice, $mmUtil, $mmSitesManager,
	...
	console.log($cordovaDevice.getPlatform());
        var promise,
            data = notification ? notification.userdata : {};
            data.foreground = notification.foreground;
            data.title = $cordovaDevice.getPlatform().toLowerCase().title;
        ...
        return $mmSitesManager.getSitesIds().then(function (sites) {
            ...
            return $q.all(promises).then(function (counters) {
                ...
                return $q(function(resolve){
            	    window.plugins.pushNotification.setApplicationIconBadgeNumber(total);
            	    resolve(total);
                });
            });
        });
